...
describe('setTeam', () => {
  ...
  it('sets the team’s members by ID', async () => {
    ...
    const after = await setTeam('starboard', [anne, mary], before, true)
  ...
  it('sets the team’s members by Actor', async () => {
    const before = initCrawlState()
    const after = await setTeam('starboard', [{ id: anne } as Actor, { id: mary } as Actor], before, true)
    expect(after.crew.teams.starboard.crew).toEqual([anne, mary])
    expect(after).not.toBe(before)
  })

  it('won’t put anyone on two teams', async () => {
    const before = await setTeam('starboard', [anne, mary], initCrawlState(), true)

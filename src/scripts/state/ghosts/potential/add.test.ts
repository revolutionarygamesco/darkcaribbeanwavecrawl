...
describe('addPotentialGhost', () => {
  ...
  it('adds a new potential ghost to the state', async () => {
    ...
    const { ghosts } = await addPotentialGhost(ghost, state, false)
    expect(ghosts.potential).toEqual([ghost])
  })

  it('can add several new potential ghosts to the state', async () => {
    const g1 = completeGhost()
    const g2 = completeGhost()
    const { ghosts } = await addPotentialGhost([g1, g2], state, false)
    expect(ghosts.potential).toEqual([g1, g2])
  })
  ...
  it('retains previous potential ghosts', async () => {
    ...
    const { ghosts } = await addPotentialGhost(g2, state, false)
    expect(ghosts.potential).toEqual([g1, g2])
  })
})
